...
public:
    ...
    bool insertAtPosition(int value, size_t position) {
        if (isFull() || position > size) return false;
        if (position == 0) return insertAtBeginning(value);
        if (position == size) return insertAtEnd(value);
        Node* newNode = new Node(value);
        Node* current = head;
    ...
    bool deleteAtPosition(size_t position) {
        ...
        if (position == 0) return deleteAtBeginning();
